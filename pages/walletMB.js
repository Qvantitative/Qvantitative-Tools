...
import {BellIcon, HomeIcon, FolderIcon, XMarkIcon} from '@heroicons/react/24/outline'
import {
  useMoralisWeb3Api,
  useNewMoralisObject} from "react-moralis";
...
export default function WalletMB() {
                  ...
                  {/* Mobile menu button */}
                    <Disclosure.Button className="bg-gray-800 inline-flex items-center justify-center p-2 rounded-md text-gray-400 right-0 hover:text-white focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white">
                      ...
                      {open ? (
                        ...
                        <FolderIcon className="block h-6 w-6 absolute inset-y-7 right-0" aria-hidden="true" />
                      )}
                    </Disclosure.Button>
                  </div>
                                            ...
                                            .then(data => {
                                              ...
                                              if (arrIntersection.length !== 0) {
                                                ...
                                                const responses = arrIntersection.map((arrIntersection) =>
                                                    fetch(`${baseURL1}${arrIntersection.ownerAddress}/collections/v2?collection=0x23581767a106ae21c074b2276D25e5C3e136a68b&includeTopBid=false&offset=0`)
                                                        .then((res) => res.json()),
                                                );
                                                Promise.all(responses)
                                                    .then(fetchedOrders => {
                                                      ...
                                                      new Chartjs(ctx,{
                                                        ...
                                                        options: {
                                                          ...
                                                          plugins: {
                                                            ...
                                                            title: {
                                                              display: true,
                                                              text: `${amount} Moonbirds owned by ${wallet.collection.slug} holders`
                                                            }
                                                          }
                                                        },
                                                      });
                                      ...
                                      <div className="wallet " key={index}>
                                        <div className={filterData % 2 === 0 ? 'bg-white' : 'bg-gray-50 transition ease-in-out delay-150 duration-300 px-4 py-5 bg-white shadow rounded-lg overflow-hidden sm:p-8' }>
                                          <div className="whitespace-nowrap text-sm font-medium text-gray-500">
                                            <button onClick={getMoonBirds} disabled={loading}>
                                              <img
